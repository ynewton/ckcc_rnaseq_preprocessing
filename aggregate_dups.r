...
rowMean <- function(i, m, t){
	subset <- m[rownames(m) == i,]
	if(t == "mean"){
		combined <- apply(subset, 2, mean)
	}else{
		combined <- apply(subset, 2, median)
	}
	return(combined)
}
...
main <- function(argv) {  
	...
	agg_type <- "mean"
	if(length(argv) == 3){
		agg_type <- tolower(argv[3])
		if(!(agg_type %in% c("mean","median"))){
			write("ERROR: invalid aggregation method is specified", stderr());
			q();		
		}
	}
	...
	if(nrow(data_matrix.true) == 0){
		...
		combined.rows <- t(rbind(sapply(d.n, rowMean, m=data_matrix.true, t=agg_type)))
